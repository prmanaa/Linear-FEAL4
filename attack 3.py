...
def linear_approximation(P_H, P_L, C_H, C_L, k1):
    left_side = (P_H >> 2 & 1) ^ (P_H >> 8 & 1) ^ (P_H >> 10 & 1) ^ (P_H >> 16 & 1)
    left_side ^= (P_L >> 8 & 1) ^ (C_H >> 2 & 1) ^ (C_H >> 8 & 1) ^ (C_H >> 10 & 1) ^ (C_H >> 16 & 1)
    left_side ^= (C_L >> 8 & 1) ^ (feal.fM(k1, P_H ^ P_L) >> 8 & 1)
    return left_side == 0
...
def find_best_k1(filename):
    ...
    # Iterate over all possible 11-bit combinations for bits 8-18
    for bits in range(0, 2**14):
        # Construct k1 by setting only bits 8-18
        k1 = (bits << 8)  # Shift the 11 bits to positions 8-18
