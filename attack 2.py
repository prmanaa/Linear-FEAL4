...
def linear_approximation(P_L, P_R, C_L, C_R, k1):
    left_side = (P_L >> 2 & 1) ^ (P_L >> 8 & 1) ^ (P_R >> 0 & 1) ^ (C_L >> 2 & 1) ^ (C_L >> 8 & 1) ^ (C_R >> 0 & 1)
    left_side ^= (feal.fM(k1, P_L ^ P_R) >> 0 & 1)
    return left_side == 0
...
def find_best_k1(filename):
    ...
    # Iterate over all possible 14-bit combinations for bits 0-13
    for bits in range(0, 2**14):
        # Construct k1 using only the 14 bits (0-13)
        k1 = bits  # Bits 0-13 are directly used
