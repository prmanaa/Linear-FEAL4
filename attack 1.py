...
def linear_approximation(P_L, P_R, C_L, C_R, k1):
    left_side = (feal.fM(k1, P_L ^ P_R) >> 16 & 1) ^ ((P_L >> 10 & 1) ^ (P_L >> 16 & 1) ^ (P_L >> 18 & 1) ^ (P_L >> 26 & 1))
    left_side ^= (P_R >> 10 & 1) ^ (P_R >> 18 & 1) ^ (P_R >> 26 & 1) ^ (C_R >> 16 & 1)
    left_side ^= (C_L >> 10 & 1) ^ (C_L >> 16 & 1) ^ (C_L >> 18 & 1) ^ (C_L >> 26 & 1)
...
def find_best_k1(filename):
    ...
    # Iterate over all possible 14-bit combinations for bits 8-21
    for bits in range(0, 2**14):
        # Construct k1 by setting only bits 8-21
        k1 = bits << 8
